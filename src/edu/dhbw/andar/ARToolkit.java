...
public class ARToolkit {
	...
	private Vector<edu.dhbw.andar.ARObject> arobjects = new Vector<edu.dhbw.andar.ARObject>();
	/**
	 ...
	 */
	public synchronized void registerARObject(edu.dhbw.andar.ARObject arobject)
	...
	public synchronized void unregisterARObject(edu.dhbw.andar.ARObject arobject) {
		if(arobjects.contains(arobject)) {
	 ...
	 */
	private native void addObject(int id, edu.dhbw.andar.ARObject obj, String patternName, double markerWidth, double[] markerCenter);
	...
	private void initialize() {
		...
		if(screenWidth>0 && screenHeight>0&&imageWidth>0&&imageHeight>0) {
			...
			edu.dhbw.andar.ARObject.glCameraMatrixBuffer = GraphicsUtil.makeFloatBuffer(edu.dhbw.andar.ARObject.glCameraMatrix);
			if(Config.DEBUG)
	...
	public final void draw(GL10 gl) {
		if(initialized) {
			...
			for (edu.dhbw.andar.ARObject obj : arobjects) {
				if(obj.isVisible())
					obj.draw(gl);
			}
		}
