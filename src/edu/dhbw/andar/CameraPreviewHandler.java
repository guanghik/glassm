...
public class CameraPreviewHandler implements PreviewCallback {
	...
	protected void init(Camera camera)  {
		...
		if (camParams.getPreviewFormat() == PixelFormat.YCbCr_420_SP)  {
			...
			throw new AndARRuntimeException("error_unkown_pixel_format");
		}	
	...
	class ConversionWorker extends Thread {
		...
		public synchronized void run() {	
			...
			while(true) {
				...
				if(Config.USE_ONE_SHOT_PREVIEW && edu.dhbw.andar.CameraPreviewHandler.this.constFPS != null) {
					//we may get a new frame now.
					synchronized(edu.dhbw.andar.CameraPreviewHandler.this.constFPS) {
						edu.dhbw.andar.CameraPreviewHandler.this.constFPS.notify();
					}					
				}
		...
		public synchronized void run() {
			...
			while(threadsRunning) {
				...
				if(camStatus.previewing) {
					try {
						cam.setOneShotPreviewCallback(edu.dhbw.andar.CameraPreviewHandler.this);
